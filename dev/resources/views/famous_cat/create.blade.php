...
<head>
	...
	<script src="{{ asset('/js/FamousCat/create.js')  . $ver_str}} }}" defer></script>
	...
	<link href="{{ asset('/css/FamousCat/create.css')  . $ver_str}}" rel="stylesheet">
	
	<title>有名猫管理・新規登録フォーム</title>
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
	...
	<li class="breadcrumb-item"><a href="{{ url('famous_cat') }}">有名猫管理・一覧</a></li>
	<li class="breadcrumb-item active" aria-current="page">有名猫管理・新規登録フォーム</li>
  </ol>
</nav>
...
<div>
	<div class="form_w" >
		<form id="form1" method="POST" action="{{ url('famous_cat/store') }}" onsubmit="return checkDoublePress()" enctype="multipart/form-data">
			@csrf
			...
			<div class="row">
				...
				<div class="col-12 col-md-7">
					<input name="neko_name" type="date"  class="form-control form-control-lg" placeholder="neko_name" value="{{old('neko_name', $ent->neko_name)}}">
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-12 col-md-7">
					<input name="neko_date" type="text"  class="form-control form-control-lg" placeholder="neko_date" value="{{old('neko_date', $ent->neko_date)}}" pattern="[0-9]{4}(-|/)[0-9]{1,2}(-|/)[0-9]{1,2} [0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2}" title="日時（Y-m-d H:i:s)を入力してください。(例  2012-12-12 12:12:12)">
				</div>
			</div>

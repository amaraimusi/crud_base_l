...
class FamousCatController extends CrudBaseController{
	...
	public function index(Request $request){
		...
		$sesSearches = session('famous_cat_searches_key');// セッションからセッション検索データを受け取る
		...
		if(!empty($request->all()) || $new_version == 1){
			$searches = [
					...
					// CBBXS-6000
					'neko_val1' => $request->neko_val1, // neko_val・範囲1
					'neko_val2' => $request->neko_val2, // neko_val・範囲2
					'neko_name' => $request->neko_name, // neko_name
					'neko_date_ym' => $request->neko_date_ym, // neko_date・年月
					'neko_date1' => $request->neko_date1, // neko_date・範囲1
					'neko_date2' => $request->neko_date2, // neko_date・範囲2
		...
		session(['famous_cat_searches_key' => $searches]); // セッションに検索データを書き込む
		...
		$crudBaseData = [
				...
				'paths'=>$paths,
				'fieldData'=>$fieldData,
				'model_name_c'=>'FamousCat', // モデル名（キャメル記法）
				'model_name_s'=>'famous_cat', // モデル名（スネーク記法）
				'def_per_page'=>$def_per_page, // デフォルト制限行数
				'this_page_version'=>$this->this_page_version,
				'new_version' => $new_version,
				
				// CBBXS-6002
				'nekoTypeList'=>$nekoTypeList, // 猫種別リスト
	...
	public function regAction(){
		...
		if(empty($id)){
			$model->sort_no =$this->getNextSortNo('famous_cats', 'asc');
	...
	public function create(Request $request){
		...
		if($ent->famous_cat_dt == '0000-00-00 00:00:00') $ent->famous_cat_dt = '';
		
		$userInfo = $this->getUserInfo(); // ログインユーザーのユーザー情報を取得する
		$paths = $this->getPaths(); // パス情報を取得する
		
		// CBBXS-3037
		$nekoTypeList = $model->getNekoTypeList(); // 猫種別
		...
		$crudBaseData = [
				...
				'paths'=>$paths,
				'this_page_version'=>$this->this_page_version,
				'famous_catTypeList'=>$famous_catTypeList,
		];
		
		return view('famous_cat.create', [
				'ent'=>$ent,
				'userInfo'=>$userInfo,
				'this_page_version'=>$this->this_page_version,
				'crudBaseData' => $crudBaseData,
	...
	public function store(Request $request){
		...
		$model->img_fn = $fileUploadK->uploadForLaravelMpa('famous_cat', $_FILES,  $ent, 'img_fn', 'img_fn_exist');

		$model->update(); // ファイル名をモデルにセットしたのでモデルをDB更新する。
		
		return redirect('/famous_cat');
	...
	public function show(Request $request){
		...
		$ent = FamousCat::find($id);
		
		// CBBXS-6037
		$nekoTypeList = $model->getNekoTypeTypeList(); // 猫種別リスト
	...
	public function update(Request $request){
		...
		$model = FamousCat::find($request->id);
	...
	public function csv_download(){
		...
		$searches = session('famous_cat_searches_key');// セッションからセッション検索データを受け取る

...
$(()=>{
	...
	rowExchange = new RowExchange('main_tbl', data, null, (param)=>{
		...
		let auto_save_url = 'famous_cat/auto_save';
...
function disabledBtn(btnElm, action_flg){
	...
	let url = 'famous_cat/disabled'; // Ajax通信先URL
...
function destroyBtn(btnElm){
	...
	let json_str = JSON.stringify(data);//データをJSON文字列にする。
	let url = 'famous_cat/destroy'; // Ajax通信先URL
...
function regAction(){
	...
	// SPA型・登録アクション
	crudBase.regAction(ent,'famous_cat/reg_action', {
		'callback': (param)=>{
			// DB登録後のコールバック処理内容
...
function clearA(){
	...
	location.href = 'famous_cat?clear=1';
}
